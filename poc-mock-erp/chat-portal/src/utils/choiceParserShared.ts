/**
 * 通用 choice 解析（与 gateway choiceParserShared 规则一致）
 */

import { normalizeChoiceText } from "./choiceText";

/** 中台 live 常输出弯引号 “ ”，归一化为直引号便于解析 */
export function normalizePlatformQuotes(text: string) {
  return String(text ?? "")
    .replace(/[\u201c\u201e\u2033]/g, '"')
    .replace(/[\u201d\u201f\u2036]/g, '"')
    .replace(/[\u2018\u201a]/g, "'")
...
export type ChoiceOption = { id: string; label: string; message: string };
...
export type ParsedChoiceBlocks = {
  intro?: string;
  blocks: ChoiceBlock[];
  footer?: string;
};
...
function isTrivialHint(hint: string) {
  const t = hint.replace(/\s/g, "");
  return !t || /^[。.，,、；;：:]+$/.test(t);
}

function splitInlineNumberedOptions(line: string) {
...
function isPureEntityOptionLine(line: string) {
...
function buildOptions(label: string, optionLabels: string[], groupIndex: number): ChoiceOption[] {
...
function processLine(line: string, hintParts: string[], optionLines: string[]) {
...
function buildChoiceBlock(label: string, body: string, groupIndex: number): ChoiceBlock | null {
  ...
  let hint = normalizeChoiceText(hintParts.join(" ").trim());
...
function isTrivialBracketReferenceBody(body: string) {
  const t = body.trim();
  if (!t) return true;
...
export function parseBracketSections(text: string): ParsedChoiceBlocks | null {
  ...
  return { intro: introText || undefined, blocks, footer: undefined };
}

export function parseLegacySections(text: string): ParsedChoiceBlocks | null {
  const lines = text.split(/\r?\n/).map(stripMarkdownLine).filter(Boolean);
  const intro: string[] = [];
  const footer: string[] = [];
  const groups: { label: string; hint: string; optionLines: string[] }[] = [];
  let current: { label: string; hint: string; optionLines: string[] } | null = null;
  let seenGroup = false;

  for (const line of lines) {
    const optMatch = line.match(OPTION_RE);
    if (optMatch) {
      if (!current) current = { label: "请选择", hint: "", optionLines: [] };
    ...
    if (fieldMatch && CONFIRM_HINT_RE.test(line)) {
      if (current?.optionLines.length || current?.hint) groups.push(current);
      current = { label: fieldMatch[1].trim(), hint: fieldMatch[2].trim(), optionLines: [] };
      seenGroup = true;
      ...
      if (quoted.length >= 2) current.optionLines.push(...quoted);
      continue;
    }
  ...
  const blocks = groups
    ...
    .filter((b) => b !== null) as ChoiceBlock[];
...
export function hasLineStartBracketFieldSections(text: string) {
...
export function isSingleNearMatchHint(hint?: string) {
  return SINGLE_NEAR_MATCH_HINT_RE.test(String(hint || ""));
}

export function extractSingleNearMatchName(hint?: string) {
...
export function buildYesNoMatchOptions(matchedName: string, groupIndex: number) {
...
function trimHintBeforeConfirm(hint?: string) {
  const h = String(hint || "");
  const idx = h.search(/请您确认[：:]/);
  return idx >= 0 ? h.slice(0, idx).trim() : h.trim();
}

export function applyShipmentYesNoMatchOptions(parsed: ParsedChoiceBlocks | null): ParsedChoiceBlocks | null {
  ...
  const blocks = parsed.blocks.map((block, groupIndex) => {
    ...
    if (confirmTail && !footer) footer = confirmTail;
    return {
      ...block,
