/**
 * 场景2（智能发货申请单）专用文本解析（与 gateway shipmentChoiceParser 规则一致）
 */
...
function cleanShipmentHint(body: string) {
...
function normalizeShipmentIntro(intro?: string) {
...
function splitIntroAndNumberedOptions(text: string) {
...
function extractNarrativeSectionOption(line: string) {
...
function isNarrativeContextLine(line: string) {
...
function isSectionContextHintLine(line: string) {
  const s = stripLine(line);
...
function isQuestionOrPromptLine(line: string) {
  const s = stripLine(line);
  return /[？?]$/.test(s) || /是否应为|请选择哪一个|是否为其他|还是另有/.test(s);
}

function extractStandaloneEntityOption(line: string) {
  const stripped = stripLine(line);
...
function normalizeReviewSectionHeaders(text: string) {
  return normalizePlatformQuotes(String(text || ""))
    .replace(/(?:^|\n)(【客户名称】)\s*/gm, "\n客户名称：")
    .replace(/(?:^|\n)(【商品名称】)\s*/gm, "\n商品名称：");
}

function findConfirmBlockStart(text: string) {
  const m = String(text).match(/(?:^|\n\n?)(?:请确认|请选择)[：:]?\s*(?:\n|$)/m);
  return m ? m.index! : -1;
}

function parseConfirmQuestionOptions(text: string) {
...
export function isReviewConfirmClarifyText(text: string) {
...
function extractShipmentEntityOption(line: string) {
  const stripped = stripLine(line);
  if (!stripped || isSectionHeaderLine(stripped)) return null;
...
function parseShipmentSectionContent(rawText: string) {
...
function isShipmentSectionAmbiguityFormat(text: string) {
  return isReviewConfirmClarifyText(text);
}

function parseShipmentSectionAmbiguity(text: string): ParsedChoiceBlocks | null {
  ...
  const productRaw = productAbs >= 0 ? t.slice(productAbs, confirmEnd) : "";
...
function uniqueChoiceOptions(labels: string[], groupIndex: number) {
...
function classifyClarifyOption(text: string) {
...
function candidateNamesFromLine(line: string, preferLastQuote = false) {
...
function extractProductHint(intro: string) {
...
export function isShipmentClarifyFormat(text: string) {
  const t = String(text || "");
  if (isShipmentMissingFormat(t)) return false;
...
export function parseShipmentClarifySections(text: string): ParsedChoiceBlocks | null {
  if (!text) return null;
  ...
  const productOptionLabels: string[] = [];
...
export function isShipmentMissingFormat(text: string) {
  const t = String(text || "");
...
function parseShipmentInlineMissing(text: string): ParsedChoiceBlocks | null {
...
export function parseShipmentMissingSections(text: string): ParsedChoiceBlocks | null {
  if (!text) return null;
...
/** 场景2 统一入口 */
export function tryParseShipmentChoiceBlocks(text: string): ParsedChoiceBlocks | null {
  const t = String(text || "").trim();
  if (!t) return null;

  let parsed: ParsedChoiceBlocks | null = null;
