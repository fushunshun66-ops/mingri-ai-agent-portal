...
function docFootHint(opts: {
  ...
  chipFilled: boolean;
  hasImmediateConfirm: boolean;
  hasModifyActions: boolean;
}) {
  if (opts.confirmFilled) return "已提交";
  if (opts.chipFilled) return "已填入输入框，确认后发送";
  if (opts.hasImmediateConfirm && opts.hasModifyActions) {
    return TEMP_DISABLE_MODIFY_DOC_ACTIONS
      ? "确认将直接发送"
      : "确认将直接发送；修改项填入输入框后再发送";
  }
  if (opts.hasImmediateConfirm) return "点击后将直接发送";
  return "点击后将选项填入下方输入框";
}
  ...
  return (
    <div className={`doc-card cap-accent-blue`}>
      <div className="doc-card-head">
        ...
        <Badge
          variant="outline"
          className="text-[11px] font-semibold px-[10px] py-[4px] rounded bg-[#fff7e6] text-[#d48806] border-[#ffd591]"
        >
          待确认
        </Badge>
      </div>
      ...
      {actions && actions.length > 0 ? (
        ...
        <DocCardFoot
          hint={docFootHint({
            confirmFilled: fallbackFilled,
            chipFilled: false,
            hasImmediateConfirm: true,
          ...
          actions={
            <DocActionButton
              ...
              disabled={disabled || fallbackFilled}
              onClick={() =>
                triggerDocAction(onAction, {
                  messageId,
                  blockIndex,
