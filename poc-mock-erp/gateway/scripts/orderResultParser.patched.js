/**
 * Patched orderResultParser with enricher integration.
 */
import { looksLikeStructuredDocJson, parseLooseJson } from "./jsonPayload.js";
import { enrichOrderResultBlock } from "./orderResultEnricher.js";
...
function buildResult(orderNo, messageOverride, sourceValue) {
...
export function tryOrderResultFromObject(value) {
  ...
  const orderNoRaw = value.订单号 ?? value.订单编号 ?? value.orderNo ?? value.order_no ?? value.单号 ?? value.orderCode;
  const orderNo = normalizeOrderNo(orderNoRaw);
  if (!orderNo) return null;

  const messageText = value.message != null ? String(value.message).trim() : "";
  const statusText = String(value.状态 ?? value.status ?? value.结果 ?? "");
  const blob = `${statusText} ${messageText} ${JSON.stringify(value)}`;

  if (value.success === true || SUCCESS_HINT_RE.test(blob) || PLAIN_ORDER_NO_RE.test(String(orderNoRaw).trim())) {
    return buildResult(orderNo, messageText || undefined, value);
  }
