 ...
 * 支持 live 叙述式（含/不含「关于」前缀）、编号列表、多问题并列。
 */
...
import { tryParseShipmentChoiceBlocks, isReviewConfirmClarifyText } from "./shipmentChoiceParser.js";
...
function extractProductOptionsFromHint(hint) {
...
/** live 叙述式：字段引号 + 相似/未匹配/编号列表 */
export function isSalesOrderNarrativeClarifyFormat(text) {
  const t = String(text || "");
...
function parseSectionBody(body) {
  ...
  for (const line of lines) {
    ...
    if (num) {
      ...
      if (label && /有限公司|股份有限公司|集团/.test(label)) {
        options.push(label);
      } else if (label) {
        options.push(label);
      }
...
function splitNarrativeFieldSections(text) {
...
function parseSalesOrderNarrativeSections(text) {
...
function hasClarifyChoiceContent(text) {
...
/** 中台「缺少关键要素「字段」」— 纯提醒，无可选项 */
export function parseSalesOrderMissingFieldsReminder(text) {
...
/**
 * 销售订单 flow 专用：缺少要素提醒 → 叙述式 → 【字段】→ **字段** 旧格式
 */
export function tryParseSalesOrderChoiceBlocks(text) {
